...
await host.goto('http://127.0.0.1:8765/',{waitUntil:'domcontentloaded'});
await host.waitForSelector('#enter-hub-btn:not([disabled])',{timeout:60000});await host.click('#enter-hub-btn');
await host.waitForSelector('#mode-overlay:not(.hidden)',{timeout:60000});await host.click('[data-quick-mode=tdm]');
...
const before=await host.evaluate(()=>({seed:VF.game.mapSeed,blue:VF.game.ai.blue.length,red:VF.game.ai.red.length,mode:VF.GameModes.currentId(),runtime:document.body.dataset.gameRuntime}));
assert.equal(before.mode,'tdm');assert.equal(before.runtime,'small');
console.log('TDM',before);
await host.evaluate(()=>document.exitPointerLock&&document.exitPointerLock());await host.waitForTimeout(200);
await host.click('#game-back-btn');
await host.waitForSelector('#mode-overlay:not(.hidden)',{timeout:60000});
await host.close();
for(const mode of ['demo','ffa','gungame','core']){
 const page=await context.newPage();watch(page);
 await page.goto('http://127.0.0.1:8765/',{waitUntil:'domcontentloaded'});await page.waitForSelector('#enter-hub-btn:not([disabled])',{timeout:60000});await page.click('#enter-hub-btn');await page.waitForSelector('#mode-overlay:not(.hidden)',{timeout:60000});await page.click('[data-quick-mode="'+mode+'"]');
 ...
 assert.equal(state.mode,mode);console.log('MODE',state);
 await page.close();
}
const page=await context.newPage();watch(page);
...
// 离线无平台房间服务时 32v32 退回本地大战（SDK 迁移后不再有 localStorage 离线 PvP）。
// 断言它确实开了征服局，而不是把人留在已关掉的模式选择界面上。
await page.click('[data-mode-action=conquest32]');await page.waitForSelector('#class-overlay:not(.hidden)');assert.equal(await page.evaluate(()=>VF.game.mode),'pve');assert.equal(await page.evaluate(()=>VF.game.matchMode),'conquest');
...
const returning=await context.newPage();watch(returning);await returning.goto('http://127.0.0.1:8765/',{waitUntil:'domcontentloaded'});await returning.waitForSelector('#enter-hub-btn:not([disabled])',{timeout:60000});await returning.click('#enter-hub-btn');await returning.waitForSelector('#mode-overlay:not(.hidden)',{timeout:60000});await returning.click('[data-quick-mode=tdm]');await returning.waitForSelector('#class-overlay:not(.hidden)',{timeout:60000});await returning.click('#class-cancel-btn');await returning.waitForSelector('#mode-overlay:not(.hidden)',{timeout:60000});assert(!returning.url().includes('screen=modes'));await returning.close();console.log('CANCEL_RETURNS_SHARED_LOBBY ok');

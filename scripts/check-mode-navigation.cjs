...
for(const suffix of ['', '?screen=modes', 'index.html?screen=modes&mode=ffa']){
 await page.goto(base+suffix,{waitUntil:'domcontentloaded'});
 await page.waitForSelector('#enter-hub-btn:not([disabled])',{timeout:15000});
 assert(await page.locator('#start-overlay').isVisible());
 assert(!(await page.locator('#mode-overlay').isVisible()));
 assert(!(await page.locator('#game-entry-transition').count()));
 assert(!page.url().includes('screen=modes'));
}
console.log('HOME: fresh and stale lobby URLs show the cover.');

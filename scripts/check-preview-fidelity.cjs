...
(async()=>{const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true,args:['--enable-unsafe-swiftshader']});try{
 ...
 const out=require('path').join(process.env.VOXEL_FRONTLINE_DOCS_DIR || require('path').resolve(__dirname,'../../voxel-frontline-docs'),'docs/ui-refactor');fs.mkdirSync(out,{recursive:true});
 ...
 for(const size of [{width:1440,height:900},{width:1280,height:720},{width:800,height:600}]){
  await page.setViewportSize(size);
  const visible=await page.evaluate(()=>{const a=document.querySelector('.soldier-browser').getBoundingClientRect(),b=document.querySelector('.frontline-subnav').getBoundingClientRect();return a.top>=b.bottom&&a.bottom<=innerHeight;});assert(visible,'Archive intersects navigation');
 }
 await page.setViewportSize({width:1440,height:900});await page.screenshot({path:out+'/archive.png'});
 await page.click('[data-mode-action=mode-home]');await page.click('[data-mode-action=solo]');await page.waitForSelector('#class-overlay:not(.hidden)');

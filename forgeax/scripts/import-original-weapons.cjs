...
function exportKnives(prefix){const before=seed;for(const cls of env.VF.Soldier.CLASSES){const model=env.VF.Soldier.createKnifeViewModel(cls.id,{team:'ally'});collect(prefix+'gadget/'+cls.id+'/knife',model);
 const slash=model.userData.slash,rest={...model.userData.rest};if(!slash||!rest)throw Error('Knife pivot missing');meleePoses[prefix+cls.id]={slash:rest};
 slash.visible=false;collect(prefix+'gadget/'+cls.id+'/knife-guard',model);slash.visible=true;
 slash.removeFromParent();slash.position.set(0,0,0);slash.rotation.set(0,0,0);collect(prefix+'gadget/'+cls.id+'/knife-slash',slash);}seed=before;}
exportKnives('');
...
for(const cls of env.VF.Soldier.CLASSES){const model=env.VF.Soldier.createViewModel(cls.id,'ar');model.root.remove(model.gun);collect('small/arms/'+cls.id,model.root);
 const left=model.leftArm,rest={x:left.position.x+.3,y:left.position.y-.34,z:left.position.z-.52,rx:left.rotation.x,ry:left.rotation.y,rz:left.rotation.z};
 poses['small/arms/'+cls.id+'/left']={hip:rest,ads:rest};left.visible=false;collect('small/arms/'+cls.id+'/right',model.root);left.visible=true;left.removeFromParent();left.position.set(0,0,0);left.rotation.set(0,0,0);collect('small/arms/'+cls.id+'/left',left);}
...
// EQ-06: original skill geometry, split at the authored turret pivots.
const skillProto=env.VF.Skills.prototype;
collect('skill/healer',skillProto._makeHealDeviceMesh(false));collect('skill/healer-preview',skillProto._makeHealDeviceMesh(true));
collect('skill/shield',skillProto._makeRiotShieldMesh());
collect('skill/turret-preview',skillProto._makeTurretMesh(true));
{const root=skillProto._makeTurretMesh(false),head=root.userData.head,barrels=root.userData.barrels;head.visible=false;collect('skill/turret-base',root);head.visible=true;head.removeFromParent();head.position.set(0,0,0);barrels.visible=false;collect('skill/turret-head',head);barrels.visible=true;barrels.removeFromParent();barrels.position.set(0,0,0);collect('skill/turret-barrels',barrels);}
...
fs.writeFileSync(path.join(out,'melee-poses.ts'),'// Original knife shoulder pivots.\nexport default '+JSON.stringify(meleePoses)+';\n');

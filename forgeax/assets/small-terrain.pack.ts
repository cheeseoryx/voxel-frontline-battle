...
import {smallTerrainPackage,smallTerrainMaterialGuid} from './identity.ts';
import {Materials} from '@forgeax/engine/render';
import {ok,type Asset} from '@forgeax/engine/types';
import colors from './original/small-terrain-colors.ts';
const linear=(v:number)=>v<=.04045?v/12.92:Math.pow((v+.055)/1.055,2.4);
export default definePack({schemaVersion:'2.0.0',packageId:smallTerrainPackage,name:'体素战争 / 原版体素材质',build:()=>{

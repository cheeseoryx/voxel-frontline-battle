...
const plugin:Plugin={name:'voxel-frontline/battle',inject:['world','gameHost','physics'],apply(ctx){
 ...
 async function start(session:Session){
  ...
  try{
   ...
   s.ordnance=createOrdnance(ctx.world,s);s.skills=createSkills(ctx.world,s);s.building=createBuilding(s);s.gear=createEquipment(ctx.world,s,equipmentAssets,equipWeapon);stream(s);s.ready=true;
 ...
 const keydown=(e:KeyboardEvent)=>{
  ...
  s.keys.add(e.code);if(['Space','Tab'].includes(e.code))e.preventDefault();if(e.repeat||s.dead)return;
  ...
  if(e.code==='KeyT')s.vehicles?.repair();
  if(e.code==='KeyR'&&s.building?.active){s.building.rotate();return;}
  if(e.code==='KeyR'&&s.gear?.inventory.gun&&!s.gear.busy&&s.reload===0&&s.mag<s.weapon.magSize&&s.reserve>0){s.reload=s.weapon.reloadTime*(s.mode!=='conquest'&&s.classId==='vanguard'?.85:1);s.audio?.play('weapon.mechanic.reload_start');}
 };
 const keyup=(e:KeyboardEvent)=>{const s=active;s?.keys.delete(e.code);if(!s)return;if(e.code===(s.mode==='conquest'?'KeyG':'KeyQ'))s.gear?.releaseQuick();if(e.code==='KeyG'&&s.mode!=='conquest')s.skills?.release();};
 const mousemove=(e:MouseEvent)=>{const s=active;if(!s||document.pointerLockElement!==host.canvas)return;const sensitivity=(s.ads?.0011:.0022)*(s.gear?.turnScale??1);s.yaw-=e.movementX*sensitivity;s.pitch=Math.max(-1.5,Math.min(1.5,s.pitch-e.movementY*sensitivity));};
 const mousedown=(e:MouseEvent)=>{if(!active||getState(ctx.world,Screen).unwrap()!=='battle')return;const path=e.composedPath();if(path.some(n=>n instanceof Element&&n.closest('button,input,select,[data-combat-slot]')))return;if(document.pointerLockElement!==host.canvas){lock();return;}if(e.button===0){if(active.building?.active){active.building.place();return;}if(!active.gear?.press())active.firing=true;}if(e.button===2&&!active.gear?.secondary()&&active.gear?.canAds)active.ads=true;};
 ...
 const blur=()=>{if(active){active.keys.clear();active.firing=false;active.ads=false;active.gear?.cancel();}};
 ...
 ctx.world.addSystem(Update,{name:frame,queries:[],fn:world=>{
  ...
  const shake=s.gear?.cameraShake??0,hp=(s.pitch+Math.sin(s.elapsed*39)*shake*.045)/2,hy=(s.yaw+Math.cos(s.elapsed*31)*shake*.035)/2;world.set(s.camera,Transform,{pos:s.vehicles?.camera()||[feet[0],feet[1]+1.8,feet[2]],quat:[Math.sin(hp)*Math.cos(hy),Math.cos(hp)*Math.sin(hy),-Math.sin(hp)*Math.sin(hy),Math.cos(hp)*Math.cos(hy)]}).unwrap();world.set(s.camera,Camera,{fov:(s.ads?s.gear?.adsFov??48:70)*Math.PI/180}).unwrap();
  if(s.firing&&s.gear?.canFireGun&&!s.vehicles?.seated&&!s.interacting&&(!s.arena||s.arena.match.scoringLive())&&s.cooldown===0&&s.reload===0&&s.mag>0){s.mag--;s.shots++;s.audio?.gun(s.weaponId);const rate=s.weapon.fireRate;s.cooldown=rate<=5?rate:60/rate;if(!s.weapon.automatic)s.firing=false;const origin=position(s.camera),direction:Vec3=[-Math.sin(s.yaw)*Math.cos(s.pitch),Math.sin(s.pitch),-Math.cos(s.yaw)*Math.cos(s.pitch)];s.spawnProtection=0;const hit=s.map.raycast(origin,direction,s.weapon.range||150);const device=s.gear?.projectile.raycast(origin,direction,hit?.distance??s.weapon.range??150),range=device?.distance??hit?.distance??s.weapon.range??150;const vehicleHit=s.vehicles?.hit(origin,direction,range,s.weapon.damage);const unitHit=vehicleHit??s.arena?.hit(origin,direction,range,s.weapon.damage+s.bonusShot,s.weaponId);if(unitHit==null&&device)s.gear?.projectile.damageCharge(device.part,s.weapon.damage);s.stealthUntil=0;s.bonusShot=0;if(unitHit==null&&!device&&hit&&s.map.breakBlock(...hit.cell))s.broken++;}
  stream(s);
 }}).unwrap();

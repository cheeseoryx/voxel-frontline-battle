...
import {sounds as throwableSounds} from '../original/small-throwable-audio.ts';
...
export async function createBattleAudio(world:World,host:GameHost,b:BattleState,cancelled:()=>boolean){
 ...
 try{
  ...
  function play(id:string,p?:Vec3,loopKey?:string){if(disposed)return;const s=resolve(b.mode!=='conquest'&&sounds['small.'+id]?'small.'+id:id);if(!s?.files?.length)return;const listenerPosition=b.physics.position(b.playerBody),distance=p?Math.hypot(...p.map((n,i)=>n-listenerPosition[i])):0,max=s.maxDistance||180;if(distance>max)return;

...
const fuse:Record<EquipmentProjectileKind,number>={frag:THROWABLE_RULES.frag.fuse,flash:THROWABLE_RULES.flash.fuse,stun:THROWABLE_RULES.stun.fuse,smoke:THROWABLE_RULES.smoke.fuse,semtex:Infinity,molotov:0,charge:Infinity,rpg:7};
...
export function createEquipmentProjectiles(world:World,b:BattleState,assets:Map<string,Handle<'MeshAsset','shared'>>,onEffect:(kind:EquipmentProjectileKind,p:Vec3)=>void){
 ...
 const rocketMesh=world.allocSharedRef('MeshAsset',{...createBoxGeometry(.12,.12,.6).unwrap(),materialSlots:[{slotName:'body',defaultMaterial:b.material(7)}]});
 let closed=false;
 ...
 function launch(kind:EquipmentProjectileKind,power=1){
  ...
  const wall=b.map.raycast(eye,d,1);if(wall&&wall.distance<1)for(let i=0;i<3;i++)p[i]=wall.point[i]+wall.normal[i]*.18;
  const min=kind==='molotov'?.63:.69,speed=kind==='rpg'?38:kind==='charge'?(b.mode==='conquest'?20:22):(kind==='molotov'?17.9:20.5)*(min+(1-min)*Math.min(1,Math.max(0,power)));
  const velocity=d.map((v,i)=>v*speed+(i===1&&kind==='charge'?.9:0)) as Vec3;
  const entity=world.spawn({component:Transform,data:{pos:p,quat:[0,Math.sin(b.yaw/2),0,Math.cos(b.yaw/2)]}},{component:RigidBody,data:{type:RigidBodyTypeValue.dynamic,ccdEnabled:true,gravityScale:(kind==='rpg'?1.5:16)/9.81,linearDamping:.02,angularDamping:.2}},{component:MeshFilter,data:{assetHandle:handle(kind)}},{component:MeshRenderer,data:{materials:[]}}).unwrap();
  owned.add(entity);parts.push({entity,kind,previous:p,velocity,life:fuse[kind],age:0,admitted:false,landed:false,hp:REMOTE_RULES.health,bounceAt:-Infinity});return true;
 }
 ...
 function update(dt:number){
  ...
  for(const part of [...parts]){
   ...
   if(!part.admitted){
    ...
    const result=b.physics.backend.prepareDerivedShapeCandidate!({entity:part.entity,sourceKey:'equipment-projectile/'+part.entity,revision:1,bodyType:'dynamic',shapes:[{id:'body',revision:1,cells:[[0,0,0]],voxelSize:[.18,.2,.18],isSensor:part.kind==='rpg'||part.kind==='charge'||part.kind==='semtex'||part.kind==='molotov',restitution:part.kind==='smoke'?.35:.5,friction:.8,collisionGroups:0x00110001}],motion:{centerOfMass:part.previous,linearVelocity:part.velocity,angularVelocity:part.kind==='rpg'?[0,0,0]:[3,2,1]}});
   ...
   if(!part.landed&&part.kind==='rpg'&&len>0){
    const limit=wall?.distance??len;
    const vehicle=b.vehicles?.hit(part.previous,dir,limit,0);
    const actor=b.arena?.units.find(u=>u.alive&&u.team!=='ally'&&(()=>{const q=b.arena!.position(u),t=q.reduce((v,n,i)=>v+(n+(i===1?1:0)-part.previous[i])*dir[i],0);return t>=0&&t<=limit&&Math.hypot(...q.map((n,i)=>n+(i===1?1:0)-part.previous[i]-dir[i]*t))<.65;})());
    if(vehicle!=null)contact=part.previous.map((n,i)=>n+dir[i]*vehicle) as Vec3;else if(actor)contact=p;
   }
   ...
   if(contact&&!part.landed){
    if(part.kind==='rpg'||part.kind==='molotov'){remove(part);onEffect(part.kind,contact);continue;}
   ...
   if(part.life<=0){if(part.kind==='charge')remove(part);else explode(part);continue;}
   if(isRemoteExplosive(part.kind)){if(p[1]<-20||p[0]<-30||p[2]<-30||p[0]>b.map.width+30||p[2]>b.map.width+30){remove(part);continue;}}
   else if(!part.landed&&part.age>(part.kind==='rpg'?7:4.5)){if(part.kind==='smoke'){part.landed=true;part.life=.05;}else explode(part);continue;}
   part.previous=p;
  }
 }

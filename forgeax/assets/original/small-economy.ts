// @ts-nocheck
// Generated from modes/small-battle/js/economy.js; changes belong in the importer/original rules.
export function createEconomy(global, localStorage, document=undefined) {


  const STORAGE_KEY = 'vf_small_meta_v1';
  ...
  function releaseDesignReservation(design) {
    ...
    try {
      localStorage.setItem('vf_small_custom_tower_v2', JSON.stringify(design));
    } catch (_) {}
  }
  ...
  function readTowerDesign() {
    ...
    try {
      const raw =
        localStorage.getItem('vf_small_custom_tower_v2') || localStorage.getItem('vf_small_custom_tower_v1');
      if (raw) {

...
import {modelPackage,modelGuid} from './identity.ts';
...
export default definePack({schemaVersion:'2.0.0',packageId:modelPackage,name:'体素战争 / 原版兵种',build:()=>{
 ...
 for(const [name,data] of Object.entries(source.models)){
  ...
  for(const [i,entry] of [...palette.values()].entries()){
   ...
   materialSlots.push({slotName:'original-'+i,sourceKey:key,defaultMaterial:modelGuid(key)});
  ...
  outputs['soldier/'+name]={...result.value,materialSlots,submeshes};
 }
 return ok(outputs);
}});

...
if (!(canvas instanceof HTMLCanvasElement) || !(status instanceof HTMLElement) || !(output instanceof HTMLElement)) {
  throw new Error('M36 intelligence worker demo DOM is incomplete');
}

const falsify = new URL(location.href).searchParams.get('falsify') === '1';
const providerA = createM36Provider(M36_PROVIDER_A_ID);
const providerB = createM36Provider(M36_PROVIDER_B_ID);
const channelA = new MessageChannel();
const channelB = new MessageChannel();
const rawPortA = channelA.port1;
const rawPortB = channelB.port1;

interface M36PortTrace {
...
function createTrace(providerId: string): M36PortTrace {
  return {
    providerId,
    submittedInputs: [],
    commandKinds: [],
...
const traceA = createTrace(M36_PROVIDER_A_ID);
const traceB = createTrace(M36_PROVIDER_B_ID);
let traceC: M36PortTrace | undefined;
let providerC: M36Provider | undefined;
...
const diagnostics = globalThis as {
  __forgeaxM36Done?: boolean;
  __forgeaxM36Report?: () => unknown;
};
...
function providerReport(provider: M36Provider): unknown {
  return {
    ...provider.trace,
...
function portReport(trace: M36PortTrace | undefined): unknown {
...
function publish(): void {
  ...
  if (failure !== undefined) {
    ...
    status.textContent = 'M36 failed';
  } else if (done && result?.falsifierCaught === true) {
    status.dataset.status = 'falsifier-caught';
    status.textContent = 'M36 falsifier caught';
  } else if (done && result?.ok === true) {
    status.dataset.status = 'passed';
    status.textContent = 'M36 passed';
  } else {
    status.dataset.status = 'running';
    status.textContent = 'M36 Engine Worker running';
  }
  diagnostics.__forgeaxM36Done = done;
}
...
function publishHostCloseDetail(rejected: boolean): void {
  ...
  rawPortA.postMessage({
    kind: 'm36-host-binding-closed',
    origin: 'host-binding.after-poll-command-fault',
...
function startTargetHostCleanup(): void {
  ...
  traceB.hostCloseOrigin = 'host-binding.after-poll-command-fault';
  traceB.pollOutstandingAtClose = traceB.pollOutstanding;
  const currentBinding = bindingB;
  if (currentBinding === undefined) {
    failure = new Error('M36 provider-B binding was not installed before poll fault report');
    publish();
    return;
  }
...
function createFreshBinding(): void {
  ...
  providerC = createM36Provider(M36_PROVIDER_C_ID);
  const channelC = new MessageChannel();
  const rawPortC = channelC.port1;
  traceC = createTrace(M36_PROVIDER_C_ID);
  bindingC = bindIntelligencePort(
    createHostPort(rawPortC, traceC),
    createIntelligenceRuntime(providerC, { limits: M36_LIMITS }),
  );
  observePort(rawPortC, traceC);
  rawPortA.postMessage({ kind: 'm36-fresh-port', port: channelC.port2 }, [channelC.port2]);
}
...
function observePort(rawPort: MessagePort, trace: M36PortTrace, eventKind?: string): void {
  rawPort.addEventListener('message', (event: MessageEvent<unknown>) => {
    ...
    if (
      ...
      if (hostCloseStarted && trace === traceB) {
    ...
    if (value.kind === 'm36-fresh-port-request') {
      createFreshBinding();
      return;
    }
    if (eventKind !== undefined && value.kind === eventKind && typeof value.event === 'string') {
      ...
      if (value.event === 'poll-fault-contained') startTargetHostCleanup();
...
observePort(rawPortA, traceA, 'm36-worker-event');
observePort(rawPortB, traceB);

const bindingA = bindIntelligencePort(
  hostPortA,
  createIntelligenceRuntime(providerA, { limits: M36_LIMITS }),
);
bindingB = bindIntelligencePort(
  hostPortB,
  createIntelligenceRuntime(providerB, { limits: M36_LIMITS }),
);
...
diagnostics.__forgeaxM36Report = report;
publish();

async function start(): Promise<void> {
  const bootstrap = new URL('/assets/m36-bootstrap.js', location.href);
  ...
  rawPortA.postMessage({ kind: 'm36-provider-b-port', port: channelB.port2 }, [channelB.port2]);
  publish();
}

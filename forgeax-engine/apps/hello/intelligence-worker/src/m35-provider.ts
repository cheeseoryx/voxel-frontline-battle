...
export const M35_PROVIDER_A_ID = 'm35.host.survivor';
export const M35_PROVIDER_B_ID = 'm35.host.session';
export const M35_PROVIDER_C_ID = M35_PROVIDER_B_ID;
...
export interface M35ProviderTrace {
  readonly id: string;
...
export interface M35Provider extends IntelligenceProvider {
  readonly activeCount: number;
  readonly closed: boolean;
  readonly trace: M35ProviderTrace;
  readonly waitForLateCallbacks: () => Promise<void>;
}

export function createM35Provider(id: string): M35Provider {
  const activities = new Map<ActivityId, PendingActivity>();
  const timers = new Set<ReturnType<typeof setTimeout>>();
  const isLateProvider = id === M35_PROVIDER_B_ID;
  let closed = false;
  let startCalls = 0;
  let closeCalls = 0;
  ...
  const provider: M35Provider = {
    ...
    start(submission: ActivitySubmission, sink: ActivitySink): Result<void, IntelligenceErrorType> {
      ...
      if (id === M35_PROVIDER_A_ID && submission.input === 'survivor') {
      ...
      } else if (id === M35_PROVIDER_B_ID && submission.input === 'control') {
        scheduleWhileOpen(activity, 'text', 15, () => activity.sink.text('control:'));
        scheduleWhileOpen(activity, 'terminal', 45, () => {
          activities.delete(activity.id);
          activity.sink.complete('control-ok');
        });
      } else if (id === M35_PROVIDER_B_ID && submission.input === 'closing') {
      ...
      } else if (id === M35_PROVIDER_C_ID && submission.input === 'fresh') {

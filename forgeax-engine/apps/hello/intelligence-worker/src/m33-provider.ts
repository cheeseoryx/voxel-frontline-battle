...
export const M33_PROVIDER_A_ID = 'm33.host.provider-a';
export const M33_PROVIDER_B_ID = 'm33.host.provider-b';
export const M33_PROVIDER_C_ID = 'm33.host.provider-c';
...
export interface M33ProviderTrace {
  readonly id: string;
...
export interface M33Provider extends IntelligenceProvider {
  readonly activeCount: number;
  readonly closed: boolean;
  readonly trace: M33ProviderTrace;
  readonly waitForLateCallbacks: () => Promise<void>;
}

export function createM33Provider(id: string): M33Provider {
  const activities = new Map<ActivityId, PendingActivity>();
  const timers = new Set<ReturnType<typeof setTimeout>>();
  const isLateProvider = id === M33_PROVIDER_B_ID;
  let closed = false;
  let startCalls = 0;
  let closeCalls = 0;
  ...
  const provider: M33Provider = {
    ...
    start(submission: ActivitySubmission, sink: ActivitySink): Result<void, IntelligenceErrorType> {
      ...
      if (id === M33_PROVIDER_A_ID && submission.input === 'survivor') {
      ...
      } else if (id === M33_PROVIDER_B_ID && submission.input === 'closing') {
      ...
      } else if (id === M33_PROVIDER_C_ID && submission.input === 'fresh') {

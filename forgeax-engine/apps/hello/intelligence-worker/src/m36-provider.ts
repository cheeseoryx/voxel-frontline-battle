...
export const M36_PROVIDER_A_ID = 'm36.host.survivor';
export const M36_PROVIDER_B_ID = 'm36.host.poll-fault';
export const M36_PROVIDER_C_ID = M36_PROVIDER_B_ID;
...
export interface M36ProviderTrace {
  readonly id: string;
...
export interface M36Provider extends IntelligenceProvider {
  readonly activeCount: number;
  readonly closed: boolean;
  readonly trace: M36ProviderTrace;
  readonly waitForLateCallbacks: () => Promise<void>;
}

export function createM36Provider(id: string): M36Provider {
  const activities = new Map<ActivityId, PendingActivity>();
  const timers = new Set<ReturnType<typeof setTimeout>>();
  const isLateProvider = id === M36_PROVIDER_B_ID;
  let closed = false;
  let startCalls = 0;
  let closeCalls = 0;
  ...
  const provider: M36Provider = {
    ...
    start(submission: ActivitySubmission, sink: ActivitySink): Result<void, IntelligenceErrorType> {
      ...
      if (id === M36_PROVIDER_A_ID && submission.input === 'survivor') {
        scheduleWhileOpen(activity, 'text', 20, () => activity.sink.text('survivor:'));
        scheduleWhileOpen(activity, 'terminal', 125, () => {
          activities.delete(activity.id);
          activity.sink.complete('survivor-ok');
        });
      } else if (id === M36_PROVIDER_B_ID && submission.input === 'poll-fault-target') {
        recordEmission(activity, 'text');
        activity.sink.text('staged:');
        scheduleLate(activity, 'late-text', 40, () => activity.sink.text('late-provider:'));
        scheduleLate(activity, 'late-terminal', 85, () => activity.sink.complete('late-provider-ok'));
      } else if (id === M36_PROVIDER_C_ID && submission.input === 'fresh') {

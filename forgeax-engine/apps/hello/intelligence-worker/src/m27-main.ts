...
const diagnostics = globalThis as {
  __forgeaxM27Done?: boolean;
  __forgeaxM27Report?: () => unknown;
};
...
function publish(): void {
  ...
  if (failure !== undefined) {
    ...
    status.textContent = 'M27 failed';
  } else if (done && result?.falsifierCaught === true) {
    status.dataset.status = 'falsifier-caught';
    status.textContent = 'M27 falsifier caught';
  } else if (done && result?.ok === true) {
    status.dataset.status = 'passed';
    status.textContent = 'M27 passed';
  } else {
    status.dataset.status = 'running';
    status.textContent = 'M27 Engine Worker running';
  }
  diagnostics.__forgeaxM27Done = done;
}
...
channel.port1.addEventListener('message', (event: MessageEvent<unknown>) => {
  ...
  if (value.kind !== 'm27-worker-event' || typeof value.event !== 'string') return;
...
diagnostics.__forgeaxM27Report = report;
publish();

async function start(): Promise<void> {
  const bootstrap = new URL('/assets/m27-bootstrap.js', location.href);

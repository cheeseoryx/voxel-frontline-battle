...
if (!(canvas instanceof HTMLCanvasElement) || !(status instanceof HTMLElement) || !(output instanceof HTMLElement)) {
  throw new Error('M34 intelligence worker demo DOM is incomplete');
}

const falsify = new URL(location.href).searchParams.get('falsify') === '1';
const providerA = createM34Provider(M34_PROVIDER_A_ID);
const providerB = createM34Provider(M34_PROVIDER_B_ID);
const channelA = new MessageChannel();
const channelB = new MessageChannel();
const rawPortA = channelA.port1;
const rawPortB = channelB.port1;

interface M34PortTrace {
...
function createTrace(providerId: string): M34PortTrace {
  return {
    providerId,
    submittedInputs: [],
    commandKinds: [],
...
const traceA = createTrace(M34_PROVIDER_A_ID);
const traceB = createTrace(M34_PROVIDER_B_ID);
let traceC: M34PortTrace | undefined;
let providerC: M34Provider | undefined;
...
const diagnostics = globalThis as {
  __forgeaxM34Done?: boolean;
  __forgeaxM34Report?: () => unknown;
};
...
function providerReport(provider: M34Provider): unknown {
  return {
    ...provider.trace,
...
function portReport(trace: M34PortTrace | undefined): unknown {
...
function publish(): void {
  ...
  if (failure !== undefined) {
    ...
    status.textContent = 'M34 failed';
  } else if (done && result?.falsifierCaught === true) {
    status.dataset.status = 'falsifier-caught';
    status.textContent = 'M34 falsifier caught';
  } else if (done && result?.ok === true) {
    status.dataset.status = 'passed';
    status.textContent = 'M34 passed';
  } else {
    status.dataset.status = 'running';
    status.textContent = 'M34 Engine Worker running';
  }
  diagnostics.__forgeaxM34Done = done;
}
...
function publishHostCloseDetail(rejected: boolean): void {
  ...
  rawPortA.postMessage({
    kind: 'm34-host-binding-closed',
    origin: 'host-binding.close-after-client-command-fault',
...
function startTargetHostCleanup(): void {
  ...
  traceB.hostCloseOrigin = 'host-binding.close-after-client-command-fault';
  traceB.pollOutstandingAtClose = traceB.pollOutstanding;
  const currentBinding = bindingB;
  if (currentBinding === undefined) {
    failure = new Error('M34 provider-B binding was not installed before client close report');
    publish();
    return;
  }
...
function createFreshBinding(): void {
  ...
  providerC = createM34Provider(M34_PROVIDER_C_ID);
  const channelC = new MessageChannel();
  const rawPortC = channelC.port1;
  traceC = createTrace(M34_PROVIDER_C_ID);
  bindingC = bindIntelligencePort(
    createHostPort(rawPortC, traceC),
    createIntelligenceRuntime(providerC, { limits: M34_LIMITS }),
  );
  rawPortA.postMessage({ kind: 'm34-fresh-port', port: channelC.port2 }, [channelC.port2]);
}

function observePort(rawPort: MessagePort, trace: M34PortTrace, eventKind?: string): void {
  rawPort.addEventListener('message', (event: MessageEvent<unknown>) => {
    ...
    if (
      ...
      if (hostCloseStarted && trace.providerId === M34_PROVIDER_B_ID) {
    ...
    if (value.kind === 'm34-fresh-port-request') {
      createFreshBinding();
      return;
    }
    if (eventKind !== undefined && value.kind === eventKind && typeof value.event === 'string') {
      ...
      if (value.event === 'client-close-settled' || value.event === 'client-close-rejected') {
        startTargetHostCleanup();
      }
...
observePort(rawPortA, traceA, 'm34-worker-event');
observePort(rawPortB, traceB);

const bindingA = bindIntelligencePort(
  hostPortA,
  createIntelligenceRuntime(providerA, { limits: M34_LIMITS }),
);
bindingB = bindIntelligencePort(
  hostPortB,
  createIntelligenceRuntime(providerB, { limits: M34_LIMITS }),
);
...
diagnostics.__forgeaxM34Report = report;
publish();

async function start(): Promise<void> {
  const bootstrap = new URL('/assets/m34-bootstrap.js', location.href);
  ...
  rawPortA.postMessage({ kind: 'm34-provider-b-port', port: channelB.port2 }, [channelB.port2]);
  publish();
}

...
async function waitForServer() {
  ...
  while (Date.now() < deadline) {
    try {
      const response = await fetch(`http://127.0.0.1:${port}/m36.html`);
      if (response.ok) return;
    } catch {}
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
  throw new Error('M36 preview server deadline exceeded');
}

function requiredEvent(report, name) {
  const event = report.workerEvents?.find((item) => item.event === name);
  if (event === undefined) fail(`M36 did not publish ${name}`, report.workerEvents);
  return event;
}
...
function assertWorkerTier(report) {
  ...
  if (execution?.requestedTier !== 'engine-worker' || execution?.actualTier !== 'engine-worker') {
    fail('M36 did not keep the explicit engine-worker tier', execution);
  }
  if (execution.engine?.realm !== 'worker') fail('M36 engine realm fell back to Host', execution);
  if (execution.capabilities?.worker?.available !== true) {
    fail('M36 Worker capability was not available', execution.capabilities);
  }
}
...
function assertCleanup(report) {
  ...
  for (const providerId of ['providerA', 'providerB', 'providerC']) {
    ...
    if (binding?.first?.ok !== true || binding.second?.ok !== true) {
      fail(`M36 repeated ${providerId} binding close did not settle`, report.bindingCloseResults);
    }
    if (report.bindingClosePromiseSame?.[providerId] !== true) {
      fail(`M36 ${providerId} binding close did not reuse one Promise`, report.bindingClosePromiseSame);
    }
    const provider = report[providerId];
    if (provider?.closeCalls !== 1 || provider.activeCount !== 0 || provider.closed !== true) {
      fail(`M36 ${providerId} provider cleanup proof failed`, provider);
    }
  }
  if (report.providerB?.activeCountAtClose !== 1 || report.providerB?.lateCallbackAttempts !== 2) {
    fail('M36 late provider callbacks were not quarantined after Host cleanup', report.providerB);
  }
  ...
  if (
    ...
    fail('M36 repeated client cleanup did not reuse one Promise', cleanup);
  }
  if (report.ports.providerB.postCloseHostCommands.length !== 0) {
    fail('M36 Host received a post-close command', report.ports.providerB);
  }
  if (
    report.ports.providerB.pollRequests !== 1 ||
    report.ports.providerB.maxPollOutstanding !== 1 ||
    report.ports.providerB.pollOutstanding !== 0
  ) {
    fail('M36 did not preserve one normal poll credit at the Host boundary', report.ports.providerB);
  }
}
...
function assertProof(worker) {
  ...
  if (
    ...
    fail('M36 terminal-only World mutation proof failed', proof);
  }
  const outputs = Object.values(proof.authoritativeOutputs ?? {}).sort().join('|');
  if (outputs !== 'fresh-ok|survivor-ok') fail('M36 authoritative outputs changed', proof);
  const sequences = Object.values(proof.perActivitySequences ?? {})
    .map((value) => JSON.stringify(value))
    .sort();
  if (sequences.join('|') !== '[1,2]|[1,2]') fail('M36 activity sequence proof failed', proof);
  if (proof.pendingText[worker.targetId] !== undefined) {
    fail('M36 staged target event mutated World state', proof);
  }
}

function assertCommon(report) {
  assertWorkerTier(report);
  if (report.failure !== null) fail('M36 page reported a failure', report.failure);
  if (report.freshChannelCreated !== true) fail('M36 did not create a fresh same-page channel', report);
  for (const event of [
    'initial-submitted',
    'poll-fault-contained',
...
function assertNormal(report) {
  ...
  if (worker?.ok !== true || worker.falsifierCaught !== false || worker.failure !== null) {
    fail('normal M36 Worker run did not pass', worker);
  }
  ...
  if (
    ...
    fail('normal M36 poll fault transport release was not exactly once', trace);
  }
  ...
  if (
    ...
    fail('normal M36 post-fault client contract failed', probe);
  }
  const host = worker.hostClose;
  if (
    host?.origin !== 'host-binding.after-poll-command-fault' ||
    host.pollOutstandingAtClose !== 0 ||
    ...
    fail('normal M36 explicit Host cleanup proof failed', host);
  }
  if (
    report.providerA?.starts?.map((item) => item.input).join('|') !== 'survivor' ||
    report.providerB?.starts?.map((item) => item.input).join('|') !== 'poll-fault-target' ||
    report.providerC?.starts?.map((item) => item.input).join('|') !== 'fresh'
  ) {
    fail('normal M36 provider dispatch roster changed', report);
  }
  if (report.workerEvents.some((item) => item.event === 'falsifier-caught')) {
    fail('normal M36 unexpectedly reported a falsifier', report.workerEvents);
  }
}
...
function assertFalsifier(report) {
  ...
  if (worker?.ok !== false || worker.falsifierCaught !== true || worker.failure !== null) {
    fail('M36 poll release falsifier was not caught', worker);
  }
  const trace = worker.clientTrace?.providerB;
  if (
    ...
    trace.faultThrowInjected !== true ||
    trace.stagedEventObserved !== true ||
    trace.faultStagedEventCountBefore !== 1 ||
    trace.faultStagedEventCountAfter !== 0 ||
    trace.faultPollPendingAfter !== true ||
    trace.removeListenerCalls !== 0 ||
    trace.physicalCloseCalls !== 0 ||
    trace.postFaultCommands.join('|') !== 'intelligence-submit|intelligence-cancel'
  ) {
    fail('M36 falsifier did not expose the uncontained poll release', trace);
  }
  const probe = worker.probe;
  if (
    probe?.normalClientState !== false ||
    probe.falsifierState !== true ||
    probe.uncontainedActiveCount !== 2 ||
    probe.stagedEventCountAfterFault !== 0 ||
    ...
    !String(worker.falsifierReason).includes('staged event')
  ) {
    fail('M36 falsifier reason did not name staged-event and poll-credit loss', worker);
  }
  const host = worker.hostClose;
  if (
    host?.pollOutstandingAtClose !== 0 ||
    ...
    fail('M36 falsifier did not isolate the client release failure', host);
  }
  if (report.providerB?.starts?.map((item) => item.input).join('|') !== 'poll-fault-target') {
    fail('M36 falsifier sent a post-fault command into the Host provider', report.providerB);
  }
}
...
async function runCase(browser, falsify) {
  ...
  try {
    await page.goto(`http://127.0.0.1:${port}/m36.html?falsify=${falsify ? '1' : '0'}`, {
      waitUntil: 'domcontentloaded',
    });
    await page.waitForFunction(() => globalThis.__forgeaxM36Done === true, null, {
      timeout: 30_000,
    });
    const report = await page.evaluate(() => globalThis.__forgeaxM36Report?.());
    if (report === undefined) fail('M36 page did not publish a report', report);
    if (pageErrors.length > 0) fail('M36 emitted a raw page or Worker exception', pageErrors);
    const canvas = await captureCanvas(page, `m36-${falsify ? 'falsifier' : 'normal'}.png`);
    const result = { report, canvas, consoleLogs, pageErrors };
    if (artifactDir) {
      writeFileSync(
        join(artifactDir, `m36-${falsify ? 'falsifier' : 'normal'}-raw.json`),
    ...
    const diagnostic = await page
      .evaluate(() => ({
        ...
        report: globalThis.__forgeaxM36Report?.(),
      }))
      .catch(() => undefined);
    throw new Error(
      `M36 ${falsify ? 'falsifier' : 'normal'} case failed: ${String(error)}; diagnostic=${JSON.stringify(diagnostic)}; pageErrors=${JSON.stringify(pageErrors)}; console=${JSON.stringify(consoleLogs)}`,
    );
  } finally {
    await page.close();
  }
...
try {
  ...
  try {
    ...
    if (normal.consoleLogs.some((entry) => entry.includes('404'))) {
      fail('normal M36 page emitted a missing-resource console error', normal.consoleLogs);
    }
    let falsifier;
    if (gauntlet) {
      ...
      if (falsifier.consoleLogs.some((entry) => entry.includes('404'))) {
        fail('M36 falsifier page emitted a missing-resource console error', falsifier.consoleLogs);
      }
    }
    const result = { ok: true, normal, ...(falsifier === undefined ? {} : { falsifier }) };
    if (artifactDir) {
      writeFileSync(join(artifactDir, 'm36-evidence.json'), `${JSON.stringify(result, null, 2)}\n`);
      writeFileSync(join(artifactDir, 'm36-normal-evidence.json'), `${JSON.stringify(normal, null, 2)}\n`);
      if (falsifier !== undefined) {
        writeFileSync(
          join(artifactDir, 'm36-falsifier-evidence.json'),
    ...
    process.stdout.write(
      gauntlet
        ? `M36_INTELLIGENCE_WORKER_PASS ${JSON.stringify(result)}\n`

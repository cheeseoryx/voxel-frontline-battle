...
async function waitForServer() {
  ...
  while (Date.now() < deadline) {
    try {
      const response = await fetch(`http://127.0.0.1:${port}/m34.html`);
      if (response.ok) return;
    } catch {}
    await new Promise((resolve) => setTimeout(resolve, 100));
  }
  throw new Error('M34 preview server deadline exceeded');
}

function requiredEvent(report, name) {
  const event = report.workerEvents?.find((item) => item.event === name);
  if (event === undefined) fail(`M34 did not publish ${name}`, report.workerEvents);
  return event;
}
...
function assertWorkerTier(report) {
  ...
  if (execution?.requestedTier !== 'engine-worker' || execution?.actualTier !== 'engine-worker') {
    fail('M34 did not keep the explicit engine-worker tier', execution);
  }
  if (execution.engine?.realm !== 'worker') fail('M34 engine realm fell back to Host', execution);
  if (execution.capabilities?.worker?.available !== true) {
    fail('M34 Worker capability was not available', execution.capabilities);
  }
}
...
function assertCleanup(report) {
  ...
  for (const providerId of ['providerA', 'providerB', 'providerC']) {
    ...
    if (binding?.first?.ok !== true || binding.second?.ok !== true) {
      fail(`M34 repeated ${providerId} binding close did not settle`, report.bindingCloseResults);
    }
    if (report.bindingClosePromiseSame?.[providerId] !== true) {
      fail(`M34 ${providerId} binding close did not reuse one Promise`, report.bindingClosePromiseSame);
    }
    const provider = report[providerId];
    if (provider?.closeCalls !== 1 || provider.activeCount !== 0 || provider.closed !== true) {
      fail(`M34 ${providerId} provider cleanup proof failed`, provider);
    }
  }
  if (report.providerB?.activeCountAtClose !== 1 || report.providerB?.lateCallbackAttempts !== 2) {
    fail('M34 late provider callbacks were not quarantined after Host cleanup', report.providerB);
  }
  ...
  if (
    ...
    fail('M34 repeated client cleanup did not reuse one Promise', cleanup);
  }
}
...
function assertProof(worker) {
  ...
  if (
    ...
    fail('M34 terminal-only World mutation proof failed', proof);
  }
  const outputs = Object.values(proof.authoritativeOutputs ?? {}).sort().join('|');
  if (outputs !== 'fresh-ok|survivor-ok') fail('M34 authoritative outputs changed', proof);
  const sequences = Object.values(proof.perActivitySequences ?? {})
    .map((value) => JSON.stringify(value))
    .sort();
  if (sequences.join('|') !== '[1,2]|[1,2]') fail('M34 activity sequence proof failed', proof);
}

function assertCommon(report) {
  assertWorkerTier(report);
  if (report.failure !== null) fail('M34 page reported a failure', report.failure);
  if (report.freshChannelCreated !== true) fail('M34 did not create a fresh same-page channel', report);
  for (const event of [
    'initial-submitted',
    'client-close-probed',
...
function assertNormal(report) {
  ...
  if (worker?.ok !== true || worker.falsifierCaught !== false || worker.failure !== null) {
    fail('normal M34 Worker run did not pass', worker);
  }
  const trace = worker.clientTrace?.providerB;
  if (
    trace?.closeOutcome !== 'settled' ||
    ...
    trace.postCloseCommands.length !== 0
  ) {
    fail('normal M34 client transport release was not exactly once', trace);
  }
  const probe = worker.probe;
  if (
    probe?.normalClientState !== true ||
    probe.clientCloseOutcome !== 'settled' ||
    probe.submitAfterClose?.error?.code !== 'intelligence-closed' ||
    probe.cancelAfterClose?.error?.code !== 'intelligence-closed' ||
    probe.pollAfterClose?.length !== 0
  ) {
    fail('normal M34 post-close client contract failed', probe);
  }
  const host = worker.hostClose;
  if (
    host?.origin !== 'host-binding.close-after-client-command-fault' ||
    ...
    fail('normal M34 explicit Host cleanup proof failed', host);
  }
  if (
    ...
    fail('normal M34 provider dispatch roster changed', report);
  }
  if (report.workerEvents.some((item) => item.event === 'falsifier-caught')) {
    fail('normal M34 unexpectedly reported a falsifier', report.workerEvents);
  }
}
...
function assertFalsifier(report) {
  ...
  if (worker?.ok !== false || worker.falsifierCaught !== true || worker.failure !== null) {
    fail('M34 client release falsifier was not caught', worker);
  }
  const trace = worker.clientTrace?.providerB;
  if (
    trace?.closeOutcome !== 'rejected' ||
    trace.closeCommandAttempts !== 1 ||
    trace.closeCommandThrowInjected !== true ||
    trace.removeListenerCalls !== 0 ||
    trace.physicalCloseCalls !== 0 ||
    trace.postCloseCommands.join('|') !==
      'intelligence-submit|intelligence-cancel|intelligence-poll'
  ) {
    fail('M34 falsifier did not expose the uncontained client release', trace);
  }
  const host = worker.hostClose;
  if (
    host?.pollOutstandingAtClose !== 1 ||
    ...
    fail('M34 falsifier did not isolate the client release failure', host);
  }
  if (worker.probe?.normalClientState !== false || !String(worker.falsifierReason).includes('uncontained')) {
    fail('M34 falsifier reason did not name the uncontained release', worker);
  }
}
...
async function runCase(browser, falsify, screenshotName) {
  ...
  try {
    await page.goto(`http://127.0.0.1:${port}/m34.html?falsify=${falsify ? '1' : '0'}`, {
      waitUntil: 'domcontentloaded',
    });
    await page.waitForFunction(() => globalThis.__forgeaxM34Done === true, null, {
      timeout: 30_000,
    });
    const report = await page.evaluate(() => globalThis.__forgeaxM34Report?.());
    if (report === undefined) fail('M34 page did not publish a report', report);
    if (pageErrors.length > 0) fail('M34 emitted a raw page or Worker exception', pageErrors);
    const canvas = await captureCanvas(page, `m34-${falsify ? 'falsifier' : 'normal'}.png`);
    const result = { report, canvas, consoleLogs, pageErrors };
    if (artifactDir) {
      writeFileSync(
        join(artifactDir, `m34-${falsify ? 'falsifier' : 'normal'}-raw.json`),
    ...
    const diagnostic = await page
      .evaluate(() => ({
        ...
        report: globalThis.__forgeaxM34Report?.(),
      }))
      .catch(() => undefined);
    throw new Error(
      `M34 ${falsify ? 'falsifier' : 'normal'} case failed: ${String(error)}; diagnostic=${JSON.stringify(diagnostic)}; pageErrors=${JSON.stringify(pageErrors)}; console=${JSON.stringify(consoleLogs)}`,
    );
  } finally {
    await page.close();
  }
...
try {
  ...
  try {
    ...
    if (normal.consoleLogs.some((entry) => entry.includes('404'))) {
      fail('normal M34 page emitted a missing-resource console error', normal.consoleLogs);
    }
    let falsifier;
    if (gauntlet) {
      ...
      if (falsifier.consoleLogs.some((entry) => entry.includes('404'))) {
        fail('M34 falsifier page emitted a missing-resource console error', falsifier.consoleLogs);
      }
    }
    const result = { ok: true, normal, ...(falsifier === undefined ? {} : { falsifier }) };
    if (artifactDir) {
      writeFileSync(join(artifactDir, 'm34-evidence.json'), `${JSON.stringify(result, null, 2)}\n`);
      writeFileSync(join(artifactDir, 'm34-normal-evidence.json'), `${JSON.stringify(normal, null, 2)}\n`);
      if (falsifier !== undefined) {
        writeFileSync(
          join(artifactDir, 'm34-falsifier-evidence.json'),
    ...
    process.stdout.write(
      gauntlet
        ? `M34_INTELLIGENCE_WORKER_PASS ${JSON.stringify(result)}\n`

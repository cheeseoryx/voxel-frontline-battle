...
export interface PlaySoundEffectScene {
  readonly camera: EntityHandle;
  readonly target: EntityHandle;
  readonly audioAnchor: EntityHandle;
}
...
export function buildPlaySoundEffectWorld(world: World, aspect: number): PlaySoundEffectScene {
  const targetMaterial = material(world, {
  ...
  const camera = world
    .spawn(
      {
        component: Transform,
        data: { pos: cameraPos, quat: quat.fromLookAt(quat.create(), cameraPos, [0, 0.7, 0], [0, 1, 0]) },
      },
  ...
  const target = world
    .spawn(
      ...
      { component: MeshRenderer, data: { materials: [targetMaterial] } },
    )
    .unwrap();

  const audioAnchor = world
    .spawn({
  ...
  return { camera, target, audioAnchor };
}

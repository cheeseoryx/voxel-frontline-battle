#!/usr/bin/env node
// Chrome/WebGPU smoke for the real browser move-sprite path.
...
const artifactDir = resolve(root, 'apps', 'bevy', 'move-sprite', 'artifacts');
mkdirSync(artifactDir, { recursive: true });
const vite = spawn('pnpm', ['-F', '@forgeax/bevy-move-sprite', 'dev'], { cwd: root, stdio: ['ignore', 'pipe', 'pipe'] });
let url;
...
try {
  ...
  try {
    ...
    await page.waitForFunction(() => globalThis.__bevyMoveSpriteReady === true, undefined, { timeout: 20_000 });
    ...
    const screenshotPath = resolve(artifactDir, 'move-sprite-browser.png');
    ...
    console.log('[smoke-browser] PASS - Chrome loaded the move-sprite app and reached the running-system ready marker');

#!/usr/bin/env node
// Chrome/WebGPU smoke for the real browser sprite animation path.
...
const artifactDir = resolve(root, 'apps', 'bevy', 'sprite-animation', 'artifacts');
mkdirSync(artifactDir, { recursive: true });
const vite = spawn('pnpm', ['-F', '@forgeax/bevy-sprite-animation', 'dev'], { cwd: root, stdio: ['ignore', 'pipe', 'pipe'] });
let url;
...
try {
  ...
  try {
    ...
    await page.waitForFunction(() => globalThis.__bevySpriteAnimationReady === true, undefined, { timeout: 20_000 });
    ...
    const screenshotPath = resolve(artifactDir, 'sprite-animation-browser.png');
    ...
    console.log('[smoke-browser] PASS - Chrome loaded the sprite-animation app and reached the running-system ready marker');

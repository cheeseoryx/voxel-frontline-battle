...
export interface AudioControlScene { readonly camera: EntityHandle; readonly music: EntityHandle; }
...
export function buildAudioControlWorld(world: World, aspect: number): AudioControlScene {
  ...
  const music = world.spawn({ component: AudioSource, data: { clip: HANDLE_NONE, playing: false, loop: true, volume: 1, spatialBlend: 0, bus: 'music' } }).unwrap();

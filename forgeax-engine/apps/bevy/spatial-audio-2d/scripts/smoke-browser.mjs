#!/usr/bin/env node
// Real Chrome/WebGPU/Web Audio smoke for spatial_audio_2d.
...
const artifactDir = resolve(root, 'apps', 'bevy', 'spatial-audio-2d', 'artifacts');
mkdirSync(artifactDir, { recursive: true });

const vite = spawn('pnpm', ['-F', '@forgeax/bevy-spatial-audio-2d', 'dev'], {
  cwd: root,
...
try {
  ...
  try {
    ...
    const stoppedState = await page.evaluate(() => window.__spatialAudio2dStop?.());

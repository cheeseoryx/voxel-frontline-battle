#!/usr/bin/env node
// Chrome/WebGPU smoke for the real browser glTF path.
...
const artifactDir = resolve(root, 'apps', 'bevy', 'load-gltf', 'artifacts');
mkdirSync(artifactDir, { recursive: true });
const vite = spawn('pnpm', ['-F', '@forgeax/bevy-load-gltf', 'dev'], { cwd: root, stdio: ['ignore', 'pipe', 'pipe'] });
let url;
...
try {
  ...
  try {
    ...
    await page.waitForFunction(() => window.__bevyLoadGltfReady === true, undefined, { timeout: 20_000 });
    ...
    const screenshotPath = resolve(artifactDir, 'load-gltf-browser.png');
    ...
    console.log('[smoke-browser] PASS - Chrome loaded the real glTF SceneAsset and started the app loop');
  } finally {
    await browser.close();
  }
} catch (error) {

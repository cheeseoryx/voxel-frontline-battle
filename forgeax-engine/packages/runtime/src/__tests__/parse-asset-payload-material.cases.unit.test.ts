...

{
  ...
  describe('parseAssetPayload :: material parent ref (feat-20260528 M1 / w1)', () => {
    ...
    it('(AC-06) payload with parent ref + own passes -> returns undefined (strict child contract)', () => {
      const reg = new AssetRegistry(makeMockShaderRegistry());
      const fn = makeParseFn(reg);
      ...
      // A child may override values only; root-owned passes must be inherited.
      expect(fn('material', payload, refs)).toBeUndefined();
    });

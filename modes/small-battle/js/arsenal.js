...
(function (global) {
  ...
  const Arsenal = {
    ...
    _rebuildPreview() {
      ...
      if (p.pivot) {
        ...
        p.pivot.traverse(function (child) {
          // 美术枪（根目录 ../../js/weapon-models.js）的几何体 / 材质是在它的
          // CACHE 里跨帧、跨消费点共享的一份 —— 析构会把第一人称和背枪正在用的
          ...
          mats.forEach(function (x) {
            if (x && x.dispose && !(x.userData && x.userData.shared)) x.dispose();
          });
        });
        p.pivot = null;
      }
      ...
      if (isThrow) {
        ...
        // 资产到了再重建一次（跟根目录 js/player.js 的 _awaitWeaponArt 一个套路）。
        if (def.id) this._awaitPreviewArt(def.id);
        const built = VM.buildGun(def, { art: true });
        obj = built && built.gun;
      }
      if (!obj) return;
    ...
    /**
     * 美术枪是懒加载的（根目录 ../../js/weapon-models.js）。展示界面切换枪械时
     * 资产可能还没到，此刻先画程序化枪，等资产落地再重建一次预览。同一个 id
     * 只挂一次（_previewPending 去重），并且回来时要确认面板还开着、还停在这把枪上。
     */

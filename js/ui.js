...
(function (global) {
  ...
  const UI = {
    ...
    openModeSelect() {
      ...
      if(global.VFEntry)global.VFEntry.hideTransition();
    ...
    _renderServerRows(servers) {
      ...
      for (let i = 0; i < servers.length; i++) {
        ...
        ping.textContent = server.ping == null ? '—' : server.ping + ' ms';
        ...
        if (server.runtime === 'small') { join.dataset.serverRuntime = 'small'; join.dataset.serverModeId = server.mode; }
        const pvpApi = global.VF && global.VF.Pvp;
        const available =
          server.runtime !== 'small' && pvpApi && typeof pvpApi.isServerJoinable === 'function'

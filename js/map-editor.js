...
(function (global) {
  ...
  function reletterFlags(list) {
    const src = list || state.placed;
    const flags = [];
    for (let i = 0; i < src.length; i++) {
      if (src[i] && src[i].kind === 'flag') flags.push(src[i]);
    }
  ...
  function persistKitFlags(list) {
    const flags = reletterFlags(list);
    const mapped = flags.map(function (p) {
      return {
        kind: 'flag',
        letter: p.letter,
        cx: p.cx,
    ...
    const w = world();
    const g = game();
    if (w) w._kitFlags = mapped;
    if (g) g._mapKitFlags = mapped.slice();
    return mapped;
  }
  ...
  function writeStoredKit(active) {
    if (active) state.officialLayout = false;
    const g = game();
    const payload = {
      active: !!active,
  ...
  function applyKitToWorld(placedList, opts) {
    ...
    try {
      ...
      persistKitFlags(stamped);

      if (w._rebuildAllChunks) {
        w._rebuildAllChunks({ progressive: true, syncRadius: 1 });
      } else {
  ...
  function resolvePlacedForMatch() {
    ...
    if (!state.officialLayout && state.placed && state.placed.length) return state.placed.slice();
    return [];
  }
  ...
  function applyMatchMap() {
    ...
    g._mapKitPlaced = [];
    g._mapKitTerrain = null;
    g._mapKitFlags = (w._kitFlags && w._kitFlags.slice()) || [];
    g._mapKitAiSpawns = w._aiSpawns
      ? { ally: (w._aiSpawns.ally || []).slice(), enemy: (w._aiSpawns.enemy || []).slice() }
      : { ally: [], enemy: [] };
    g._mapReadyForSeed = seed;
    state.placed = ((w._kitFlags && w._kitFlags.slice()) || []).map(function (f) {
      const x = f.cx != null ? f.cx : f.x;
      const z = f.cz != null ? f.cz : f.z;
      return {
        kind: 'flag',
        letter: f.letter,
        cx: x,
        ...
        w: GRID,
        d: GRID,
        ox: Math.floor(x - GRID * 0.5),
        oz: Math.floor(z - GRID * 0.5),
        yaw: 0,
      };
    });
  ...
  function cellNearBase(gx, gz) {
    ...
    const planned =
      (w && w._plannedBases) ||
      (global.VF && global.VF.IslandConquestMap
        ? [
            {
              x: size * global.VF.IslandConquestMap.HQ.ally.nx,
              z: size * global.VF.IslandConquestMap.HQ.ally.nz,
              gate: global.VF.IslandConquestMap.HQ.ally.gate,
            },
            {
              x: size * global.VF.IslandConquestMap.HQ.enemy.nx,
              z: size * global.VF.IslandConquestMap.HQ.enemy.nz,
              gate: global.VF.IslandConquestMap.HQ.enemy.gate,
            },
          ]
        : []);
    for (let i = 0; i < planned.length; i++) {
      const b = planned[i];
      if (Math.abs(cx - b.x) <= 34 && Math.abs(cz - b.z) <= 34) return true;
      const half = 27;
      const len = 75;
      if (b.gate === '+x' && Math.abs(cz - b.z) <= half && cx >= b.x && cx <= b.x + len) return true;
      if (b.gate === '-x' && Math.abs(cz - b.z) <= half && cx <= b.x && cx >= b.x - len) return true;
      if (b.gate === '+z' && Math.abs(cx - b.x) <= half && cz >= b.z && cz <= b.z + len) return true;
      if (b.gate === '-z' && Math.abs(cx - b.x) <= half && cz <= b.z && cz >= b.z - len) return true;
    }
    return false;
  }
  ...
  function syncCount() {
    if (state.countEl) {
      ...
      let nFlag = 0;
      for (let i = 0; i < state.placed.length; i++) {
        if (state.placed[i].kind === 'flag') nFlag++;
      }
  ...
  function redraw() {
    ...
    const useCell =
      isBridgeTool() || isZiplineTool() || isSpawnTool() || isAiSpawnTool() || isFlagTool();
  ...
  function handlePlaceAt(wx, wz) {
    if (isSpawnTool() || isAiSpawnTool() || isFlagTool()) {
      const snap = snapWorld(wx, wz);
      placeIncremental(snap.x, snap.z);
      invalidateAndRedraw();
      syncCount();
      setStatus(
        isFlagTool() ? '已放置占领点' : isAiSpawnTool() ? '已放置AI刷新点' : '已放置出生水晶'
      );
      return;
    }
  ...
  function ghostFootprint() {
    if (isBridgeTool() || isZiplineTool() || isSpawnTool() || isAiSpawnTool() || isFlagTool()) {
  ...
  function rebuildGhostMesh() {
    ...
    const h =
      isBridgeTool() || isSpawnTool() || isAiSpawnTool() || isFlagTool()
        ? 0.9
  ...
  function placeIncremental(wx, wz) {
    ...
    if (isFlagTool()) {
      if (cellNearBase(snap.gx, snap.gz)) {
        setStatus('主基地附近不可放置占领点');
        if (global.VF.UI && global.VF.UI.toast) global.VF.UI.toast('主基地不可作为占领点');
        return;
      }
      if (w && w._riverInfo) {
        const water = w._riverInfo(Math.floor(snap.x), Math.floor(snap.z));
        if (water && water.inWater) {
          setStatus('水面不可放置占领点');
          if (global.VF.UI && global.VF.UI.toast) global.VF.UI.toast('水面不可放置占领点');
          return;
        }
      }
      let nFlags = 0;
  ...
  function buildOverlayIfNeeded() {
    ...
    document.getElementById('map-kit-ridge-btn').addEventListener('click', resetRiftRidge);
